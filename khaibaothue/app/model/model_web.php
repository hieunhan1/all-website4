...
class model_web extends db{
	...
	public function _web_menu_type($type,$other=NULL){
		if($other!=NULL) $other = " AND `other`=1 ";
		$sql = "SELECT * FROM `web_menu` WHERE `delete`=0 AND status=1 AND type_id='{$type}' {$other} LIMIT 1";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		return $result->fetch_assoc();
	}

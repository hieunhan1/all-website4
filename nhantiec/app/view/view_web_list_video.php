...
<div id="article">
    ...
    $data = $this->_model->_list_web_video($current_menu['id'], $per_page, $startrow, $totalrows);
	if($totalrows>1){
		foreach($data as $row){
			$i++;
			if($i%2 == 1) $style = 'style="float:left"'; else $style = 'style="float:right"';
			echo '<div class="box_video" '.$style.'>
				<a href="'.$row['url'].'"><h3>'.$row['name'].'</h3></a>
				<div class="box_video_img"><a href="'.$row['url'].'"><img src="'.$current_menu['type_menu_img'].$row['img_avatar'].'" alt="'.$row['name'].'" /></a></div>
				<div class="box_video_info">'.$row['description'].'</div>
			</div>';
		}

...
class model_admin extends db{
	...
	*/
	...
	public function _web_menu_type(){
		$sql = "SELECT `id`,`description` as `name` FROM `web_menu_type` WHERE `status`=1 AND `type`=1 ORDER BY `order`";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	...
	public function _web_users_package($parent, $style, $arr, $where=''){
		...
		while($row = $result->fetch_assoc()){
			$arr[] = array(
				...
				'songay'=>$style.$row['songay'],
				'order'=>$style.$row['order'],
				'status'=>$row['status'],
			);
			$arr = $this->_web_users_package($row['id'], $style.'-- ', $arr, $where);
		}
		return $arr;
	}
	/*end function*/

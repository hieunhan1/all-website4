...
class modelGerenal extends modelDB{
	...
	public function _listUsersPackage($lang){
		$sql = "SELECT `id`, `name`, `price` FROM `web_users_package` WHERE `status`=1 AND `lang`='{$lang}' ORDER BY `order`";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	...
	public function _webTimeZone($limit=5){
	...
	public function _listTimeZone($id=NULL){
		$str='';

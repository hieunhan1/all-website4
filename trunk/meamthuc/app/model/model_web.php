...
class model_web extends db{
	...
	/*home*/
	public function _home_list_menu($idMenu){
		$sql = "SELECT `id`,`name`,`title` FROM `web_menu` WHERE `status`=1 AND `parent`='{$idMenu}' ORDER BY `order`";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	}
	public function _home_list_item($idMenu, $limit=NULL){
		if($limit==NULL) $limit=3;
		$sql = "SELECT `id`,`name`,`url`,`url_img`,`description` FROM `web_article` WHERE `status`=1 AND `menu_id` LIKE '%,{$idMenu},%' ORDER BY `datetime` DESC LIMIT {$limit}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	}
	public function _home_list_daubep($idMenu, $limit=NULL){
		if($limit==NULL) $limit=8;
		$sql = "SELECT `id`,`name`,`url`,`url_img`,`working` FROM `web_daubep` WHERE `status`=1 AND `menu_id` LIKE '%,{$idMenu},%' ORDER BY `datetime` DESC LIMIT {$limit}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	}
	public function _home_list_monan($idMenu, $limit=NULL){
		if($limit==NULL) $limit=8;
		$sql = "SELECT `id`,`name`,`url`,`url_img`,`daubep_id` FROM `web_monan` WHERE `status`=1 AND `menu_id` LIKE '%,{$idMenu},%' ORDER BY `datetime` DESC LIMIT {$limit}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	...
	public function _list_web_monan($id, $per_page=10, $startrow=0, &$totalrows){
		$sql = "SELECT `id`,`name`,`url`,`url_img`,`daubep_id`,`description`,`datetime`,`menu_id` FROM `web_monan` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%' ORDER BY `datetime` DESC LIMIT {$startrow}, {$per_page}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		
		$sql = "SELECT count(*) FROM `web_monan` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%'";
		$result = $this->db->query($sql);
		$row = $result->fetch_row();
		$totalrows=$row[0];
	...
	public function _list_web_daubep($id, $per_page=10, $startrow=0, &$totalrows){
		$sql = "SELECT `id`,`name`,`url`,`url_img`,`working`,`datetime`,`menu_id` FROM `web_daubep` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%' ORDER BY `datetime` DESC LIMIT {$startrow}, {$per_page}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		
		$sql = "SELECT count(*) FROM `web_daubep` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%'";
		$result = $this->db->query($sql);
		$row = $result->fetch_row();
		$totalrows=$row[0];
	...
	public function _detail_daotao($alias){
		$sql = "SELECT * FROM `web_article` WHERE `status`=1 AND `name_alias`='{$alias}' LIMIT 1";
		if(!$result = $this->db->query($sql)) return FALSE;
		if($result->num_rows != 1) return FALSE;
		return $result->fetch_assoc();
	}
	public function _detail_monan($alias){
		$sql = "SELECT * FROM `web_monan` WHERE `status`=1 AND `name_alias`='{$alias}' LIMIT 1";
		if(!$result = $this->db->query($sql)) return FALSE;
		if($result->num_rows != 1) return FALSE;
		return $result->fetch_assoc();
	}
	public function _detail_daubep($alias){
		$sql = "SELECT * FROM `web_daubep` WHERE `status`=1 AND `name_alias`='{$alias}' LIMIT 1";
		if(!$result = $this->db->query($sql)) return FALSE;
		if($result->num_rows != 1) return FALSE;
		return $result->fetch_assoc();
	}
	...
	public function _other_post_monan($id,$idMenu,$limit=5){
		$sql = "SELECT `name`,`url` FROM `web_monan` WHERE `status`=1 AND `id`<>'{$id}' AND `menu_id` LIKE '%,{$idMenu},%' ORDER BY `datetime` DESC LIMIT {$limit}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;

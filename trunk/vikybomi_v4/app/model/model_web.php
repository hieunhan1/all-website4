...
class model_web extends db{
	...
	public function _home_about($id){
		$sql = "SELECT `name`,`name_alias`,`url`,`metaDescription` FROM `web_article` WHERE `delete`=0 AND `status`=1 AND `other`=1 AND menu_id LIKE '%,{$id},%' LIMIT 1";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		return $result->fetch_assoc();
	}
	public function _home_video($id){
		$sql = "SELECT `name`,`name_alias`,`url_video` FROM `web_video` WHERE `delete`=0 AND `status`=1 AND `other`=1 AND menu_id LIKE '%,{$id},%' LIMIT 1";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		return $result->fetch_assoc();
	}
	public function _home_danhmuc($lang){
		$sql = "SELECT `id`,`name`,`url`,`url_hinh`,`type_id` FROM `web_menu` WHERE `delete`=0 AND `status`=1 AND `other`=1 AND lang='{$lang}' ORDER BY `order`";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		foreach ($result as $row) $data[] = $row;
		return $data;
	}
	public function _home_list_product($id){
		$sql = "SELECT `id`,`name`,`url`,`url_hinh`,`giaban`,`giagoc` FROM `web_product` WHERE `delete`=0 AND `status`=1 AND `other`=1 AND `menu_id` LIKE '%,{$id},%' ORDER BY `ngay_dang` DESC LIMIT 5";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		foreach ($result as $row) $data[] = $row;
		return $data;
	}
	public function _home_list_info($id){
		$sql = "SELECT `name`,`url` FROM `web_article` WHERE `delete`=0 AND `status`=1 AND `menu_id` LIKE '%,{$id},%' ORDER BY `ngay_dang` DESC LIMIT 9";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		foreach ($result as $row) $data[] = $row;
		return $data;
	}
	public function _home_list_video($id){
		$qr	= "SELECT `name`,`url` FROM `web_video` WHERE `delete`=0 AND `status`=1 AND `menu_id` LIKE '%,{$id},%' ORDER BY `ngay_dang` DESC LIMIT 9";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		foreach ($result as $row) $data[] = $row;
		return $data;
	}
	public function _properties_product_buy($id,$name,$giaban,$giaban,$giagoc=0){
		return "{$id} -|- {$name} -|- {$giaban} -|- {$giagoc}";
	}
	/*end home*/

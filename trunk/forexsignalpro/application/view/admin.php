<?php
$table = $arrUrl['currentPage'];
$navigator = $c->_model->_navigator($table);
?>
<!DOCTYPE html>
...
<div id="wrapper">
    <div id="header">
        ...
        <div id="thongtin">
            ...
            <a href="<?php echo CONS_PAGE_ADMIN;?>/user/?infomation=1">Thông tin tài khoản</a> | 
            <a href="<?php echo CONS_PAGE_ADMIN;?>/user/?logout=1">Logout</a>
        </div>
    </div>
	...
	<div id="right">
    	...
    	<div class="title" style="width:auto; float:right">
        	<a href="<?php echo CONS_PAGE_ADMIN.'/'.$table.'/?id=0';?>"><img src="<?php echo CONS_ADMIN_CSS_IMG;?>add.gif"> Thêm mới</a> &nbsp; &nbsp; 
        </div>
        <?php }?>
		...
		if(count($data)>1 && !isset($_GET['id']) && $table!='user'){
			...
			foreach($data as $row){
				...
				echo '<a href="'.CONS_PAGE_ADMIN.'/'.$table.'/?lang='.$row['code'].'" '.$style.'>'.$row['name'].'</a>|';
			}
			echo '</div>';
		}

...
<div id="content">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" id="view_select">
    	<tr bgcolor="#88C4FF">
            ...
            <th width="80">Pivot</th>
            <th width="80">Sup-S1</th>
            <th width="80">Sup-S2</th>
            <th width="80">Sup-S3</th>
            <th width="80">Res-R1</th>
            <th width="80">Res-R2</th>
            <th width="80">Res-R3</th>
            <th width="110" align="left">Datetime</th>
            <th width="90">Thao tác</th>
        </tr>
        <?php
		$i = 0;
		$data = $c->selectFromAll($lang, $arr, $table, "`{$table}`.`pivot`, `{$table}`.`support_s1`, `{$table}`.`support_s2`, `{$table}`.`support_s3`, `{$table}`.`resistance_r1`, `{$table}`.`resistance_r2`, `{$table}`.`resistance_r3`, `{$table}`.`datetime`");
		foreach($data as $row){
			$i++; ?>

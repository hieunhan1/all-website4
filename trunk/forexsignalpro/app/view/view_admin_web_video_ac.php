...
<table width="100%" border="0" cellpadding="0" cellspacing="10" style="margin-bottom:50px">';	
	...
	if($row_detail['img_avatar'] != '') $other .= '<div class="avarta"><img src="'.CONS_IMAGES_VIDEOS_THUMBS.$row_detail['img_avatar'].'" /></div>';
	...
	//type
	$arr = array();
	$arr[] = array('id'=>'1', 'name'=>'Youtube');
	$arr[] = array('id'=>'0', 'name'=>'Vimeo');
	if($row_detail['type']=='') $properties = 1; else $properties = $row_detail['type']; //default check
	$views = array('Loại video','type','radio',' &nbsp; '); //label name class other
    $this->getProperties('4',$arr,$properties,$views);
	echo $this->DisplayProperties();
	
	//url_code
	$values = $row_detail['url_code'];
	$properties = array('30'); //maxlength OTHER (disabled, readonly) 
	$views = array('Mã video','url_code','input_medium'); //label id&name class style
    $this->getProperties('1',$values,$properties,$views);
	echo $this->DisplayProperties();

...
<table width="100%" border="0" cellpadding="0" cellspacing="10" style="margin-bottom:50px">';	
	...
	//name_var
	$values = $row_detail['name_var'];
	$properties = array('30'); //maxlength OTHER (disabled, readonly) 
	$views = array('Tên biến','name_var','input_medium'); //label id&name class style
    $this->getProperties('1',$values,$properties,$views);
	echo $this->DisplayProperties();
	
	//value
	$values = $row_detail['value'];
	$properties = array('250'); //maxlength OTHER (disabled, readonly) 
	$views = array('Giá trị','value','input_medium'); //label id&name class style
    $this->getProperties('1',$values,$properties,$views);
	echo $this->DisplayProperties();

...
';
	...
	public function index(){
		...
		if( $this->_control==CONS_DEFAULT_WEB_CONTROLLER || isset($_GET['lang']) ){
			...
			$slogan = '<div style="width:400px; float:left; margin-left:25px"><h1><font color="#00A651">www.nhantiec.vn</font> <font color="#FF7C38">'.$this->_config['slogan'].'</font></h1></div>';
		}else{
			$current_menu = $this->current_menu();
			$this->_config = $this->web_config($current_menu['lang']);
			$slogan = '<div style="width:400px; float:left; margin-left:25px"><h2><font color="#00A651">www.nhantiec.vn</font> <font color="#FF7C38">'.$this->_config['slogan'].'</font></h2></div>';
		...
		$tag_head = $this->tag_head($this->_config['sitename'],$current_site['title'],$current_site['description'],$current_site['keyword'],$current_site['url'],$current_site['image'],$current_site['type_menu_name']);//end tab head
		include_once('view/view_web.php');
	}//index
	
	public function checks_video_youtube_vimeo($url_video,$domain=NULL){
		if(is_numeric($url_video)){
			$str = '<iframe id="view_video" src="//player.vimeo.com/video/'.$url_video.'" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
		}else{
			$str = '<iframe id="view_video" src="http://www.youtube.com/embed/'.$url_video.'?origin=http://'.$domain.'&amp;rel=0" frameborder="0"></iframe>';
		}
		return $str;
	}

...
class model_web extends db{
	...
	/*home*/
	public function _list_menu_home_page($lang, $type, $limit=1){
		$sql = "SELECT `id`,`name`,`url`,`img_avatar` FROM `web_menu` WHERE `status`=1 AND `other`=1 AND `lang`='{$lang}' AND `type_id`='{$type}' ORDER BY `order` LIMIT {$limit}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	...
	public function _list_article_home_page($id, $limt=1){
		$sql = "SELECT `id`,`name`,`url`,`img_avatar`,`description`,`content` FROM `web_article` WHERE `status`=1 AND `other`=1 AND menu_id LIKE '%,{$id},%' ORDER BY `datetime` DESC LIMIT {$limt}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	}
	public function _list_product_home_page($id, $limt=4){
		$sql = "SELECT `id`,`name`,`url`,`img_avatar`,`price`,`price_cost` FROM `web_product` WHERE `status`=1 AND `other`=1 AND menu_id LIKE '%,{$id},%' ORDER BY `datetime` DESC LIMIT {$limt}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	...
	public function _list_web_product($id, $per_page=10, $startrow=0, &$totalrows){
		$sql = "SELECT `id`,`name`,`url`,`img_avatar`,`price`,`price_cost` FROM `web_product` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%' ORDER BY `datetime` DESC LIMIT {$startrow}, {$per_page}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		
		$sql = "SELECT count(`id`) FROM `web_product` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%'";
		$result = $this->db->query($sql);
		$row = $result->fetch_row();
		$totalrows=$row[0];

...
?>
<div id="search">
	<form action="" method="get" name="search" onsubmit="if(search.value=='' && tinhthanh.value==0) return false;">
    	<input type="text" name="search" value="<?php if(isset($_GET['search'])) echo $_GET['search']; ?>" class="txt" placeholder="Mô tả" />
        <select name="tinhthanh" class="select">
        <?php
        $data = $this->_model->_web_ds_tinhthanh();
		foreach($data as $row){
			if($row['id']!=$_GET['tinhthanh']) echo '<option value="'.$row['id'].'">'.$row['name'].'</option>';
...
<div id="content">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" id="view_select">
        ...
        <?php
		if($_GET['tinhthanh']!=0){
			$str_search = " AND tinhthanh_id='{$_GET['tinhthanh']}' ";
			$url_search = '&tinhthanh='.$_GET['tinhthanh'];
		}

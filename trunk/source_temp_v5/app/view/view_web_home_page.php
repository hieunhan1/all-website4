...
<div id="homepage">
    ...
    <div class="box_new_market">
		...
		foreach($data as $row){
			...
			if($i!=1){
				$item .= '<div class="item">
					<a href="'.$row['url'].'" title="'.$row['name'].'"><div class="img"><img src="public/_thumbs/Images/articles/'.$row['url_img'].'" alt="'.$row['name'].'" /></div><h4>'.$row['name'].'</h4></a>
					<p>February 25, 2015 09:24:00 PM ET</p>
				</div>';
			}else{
				$highlight .= '<div class="highlight">
					<a href="'.$row['url'].'" title="'.$row['name'].'"><div class="img"><img src="public/_thumbs/Images/articles/'.$row['url_img'].'" alt="'.$row['name'].'" /></div><h3>'.$row['name'].'</h3></a>
					<p>'.$row['description'].'</p>
				</div>';
			}
		}

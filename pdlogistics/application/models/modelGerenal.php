...
class modelGerenal extends modelDB{
	...
	public function _listDetailMenu($table, $id, $limit=NULL, $select=NULL, $order=NULL){
		if($select==NULL) $select='`id`, `name`, `url`, `img`, `description`, `content`, `datetime`';
		else  $select='`id`, `name`, '.$select;
		if($limit!=NULL) $limit="LIMIT {$limit}"; else $limit='';
		if($order==NULL) $order='`datetime` DESC';
		
		$sql = "SELECT {$select} FROM `{$table}` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%' ORDER BY {$order} {$limit}";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[] = $row;
		return $data;
	...
	public function _listDataTable($table, $idMenu, &$totalRows, $perPage=10, $startRow=0, $select=NULL, $where=NULL, $orderBy=NULL){
		...
		$select = trim($select, ',');
		if($orderBy==NULL) $orderBy='`datetime` DESC';
	...
	public function _viewDetailOther($table, $id, $idMenu, $select=NULL, $where=NULL, $limit=5){
		if($select==NULL) $select='`id`, `name`, `url`'; else $select='`id`, `name`, `url`, '.$select;
	...
	/*other*/
	public function _listMenuTab($lang){
		$sql = "SELECT `id`, `name` FROM `web_menu_tab` WHERE `status`=1 AND `lang`='{$lang}' ORDER BY `order`";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[$row['id']] = $row;
		return $data;
	}
	public function _listDetailTab($id){
		$sql = "SELECT `id`, `name`, `content`, `tab_id` FROM `web_article` WHERE `status`=1 AND `menu_id` LIKE '%,{$id},%'";
		if(!$result = $this->db->query($sql)) die($this->db->error);
		$data = array();
		while($row = $result->fetch_assoc()) $data[$row['tab_id']] = $row;
		return $data;
	}
	/*end other*/
}

...
class controlGerenal{
	...
	public function getSubmenu($lang, $parent, $position){
		...
		if(count($data)>0){
			...
			foreach($data as $row){
				$res = in_array($row['id']);
				if($res!=1) $style=''; else $style='class="isExpanded"';
	...
	public function navigator($lang, $parent, $arr=NULL){
		if(!$arr) $arr=array();
		if($parent==0){
			if($lang=='vi') $arr[] = array('id'=>'', 'name'=>'Trang chủ', 'url'=>'', 'title'=>'');
			else $arr[] = array('name'=>'Home', 'url'=>'?lang=en', 'title'=>'');
			return array_reverse($arr);
		}
		$row = $this->_model->_menuIdOrAlias($parent);
		$arr[] = array('id'=>$row['id'], 'name'=>$row['name'], 'url'=>$row['url'], 'title'=>$row['title']);

...
<table width="100%" border="0" cellpadding="0" cellspacing="10" style="margin-bottom:50px">';	
	...
	echo $cF->displayTable('Mã sản phẩm', $data);
	...
	if(!isset($_POST[$name])) $value=$rowDetail[$name]; else $value=$_POST[$name];
    $data = $cF->inputText($name, $value, 'input_medium', $properties);
	echo $cF->displayTable('Tên sản phẩm', $data);
	...
	$message='<br />Upload hình ảnh vào thư mục "<strong>products</strong>"';
	...
	$name = 'price_cost';
	$properties = array();
	$properties[] = array('propertie'=>'maxlength', 'value'=>'7');
	if(!isset($_POST[$name])) $value=$rowDetail[$name]; else $value=$_POST[$name];
    $data = $cF->inputText($name, $value, 'input_medium', $properties);
	...
	echo $cF->displayTable('Đặc tính', $data);
	...
	$data = $cF->inputRadio($name, $values, $valueCheck, 'radio');
	echo $cF->displayTable('Hiện trang chủ', $data);

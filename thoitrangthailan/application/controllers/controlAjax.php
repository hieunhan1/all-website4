...
if(isset($_SESSION['adminId'])){
	...
	if(isset($_POST['updatePrice'])){
		$price = $c->_model->_changeDauNhay(trim($_POST['updatePrice']));
		$menu_id = $c->_model->_changeDauNhay(trim($_POST['menu_id']));
		if(is_numeric($price) && is_numeric($menu_id)){
			$c->_model->_updateProductPrice($menu_id, $price);
			echo 1; return true;
		}else return false;
	}
}

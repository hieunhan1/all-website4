...
class controlGerenal{
	...
	public function getSubmenu($lang, $parent, $position, $idCurrent, $adMenu){
		$data = $this->_model->_listMenu($lang, $parent, $position);
		$view='';
		if(count($data)>0){
			$view.='<ul>';
			foreach($data as $row){
				if($idCurrent!=$row['id']) $style=''; else $style='class="selected"';
				$view .= '<li '.$style.' id="'.$row['id'].'" name="'.$row['name'].'" table="web_menu"><a href="'.$row['url'].'" title="'.$row['title'].'">'.$row['name'].$adMenu.'</a>';
				$view .= $this->getSubmenu($lang, $row['id'], $position, $idCurrent, $adMenu);
				$view .= '</li>';
			}
			$view .= '</ul>';
		}
